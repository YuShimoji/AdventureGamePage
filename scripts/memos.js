(function(){
  ...
  function readMemos(){ return StorageUtil.loadJSON(KEY_MEMOS, []); }
  function writeMemos(list){ StorageUtil.saveJSON(KEY_MEMOS, list); }
  ...
  function sortMemos(list, mode){
    ...
    switch(mode){
      ...
      case 'rank_asc':
        arr.sort((a,b)=> (a.rank??0)-(b.rank??0)); break;
      case 'rank_desc':
        arr.sort((a,b)=> (b.rank??0)-(a.rank??0)); break;
      default:
  ...
  function render(listEl, memos){
    listEl.innerHTML = '';
    memos.forEach(m => {
      const wrap = document.createElement('div'); wrap.className='memo-item';
      const head = document.createElement('div'); head.className='memo-head';

      const title = document.createElement('strong'); title.textContent = m.label || 'メモ';
      const meta = document.createElement('span'); meta.className='memo-meta'; meta.textContent = new Date(m.createdAt).toLocaleString();

      const actions = document.createElement('div'); actions.className='memo-actions';
      const rankLabel = document.createElement('span'); rankLabel.textContent = '順位';
      const rankInput = document.createElement('input'); rankInput.type='number'; rankInput.value = m.rank ?? 0; rankInput.min = 0; rankInput.step = 1;
      ...
      rankInput.addEventListener('change', () => {
        const val = Number(rankInput.value)||0; m.rank = val; persistUpdate(m);
      });
      delBtn.addEventListener('click', () => { removeMemo(m.id); });
    });
  }
  ...
  function persistUpdate(updated){
    ...
    if(idx>=0){ list[idx]=updated; writeMemos(list); }
  }

  function addMemo(label, text){
    const list = readMemos();
    list.push({ id: uid(), label: label||'メモ', text: text||'', rank: 0, createdAt: new Date().toISOString() });
    writeMemos(list);
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // 初期表示時に設定が許可なら準備
    if(window.APP_CONFIG?.ui?.showMemos){ refresh(); }
  });
