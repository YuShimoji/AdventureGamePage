(function () {
  // SavePreview - Modular system implementation
  // Uses separate modules for overlay, renderer, and controls
  ...
  if (window.SavePreviewPanelManager && window.SavePreviewOverlay && window.SavePreviewRenderer && window.SavePreviewControls) {
    console.debug('[DEBUG] Using new modular SavePreview system');
    // New modular system is loaded, skip legacy code
    return;
  }

  console.debug('[DEBUG] Using legacy SavePreview system (modular system not loaded)');

  // Legacy code remains unchanged for backward compatibility
  class SavePreviewPanelManager {
    ...
    handleKeydown = (e) => {
      if (e.key === "Escape" && this.overlay.isOpen()) {
        e.preventDefault();
        this.overlay.close();
      } else if (e.key === "Tab" && this.overlay.isOpen()) {
