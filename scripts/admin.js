(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      ...
      console.warn("load editor cfg override failed", e);
    }
    ...
    async function renderSaves() {
      try {
        ...
        list.forEach((item) => {
          ...
          title.textContent = item.title || (APP_CONFIG?.strings?.defaultTitle ?? "");
    ...
    if (btnQuickZen)
      btnQuickZen.addEventListener("click", () => document.body.classList.toggle("zen-mode"));
    if (btnQuickSidebar)
    ...
    if (btnQuickPreview) {
      btnQuickPreview.addEventListener("click", () => {
        ...
        if (willOpen && typeof renderSaves === "function") {
          renderSaves();
        }
      });
    }
    ...
    try {
      if (window.StorageProviders?.Registry && providerSelect) {
        ...
        if (cfgBackend && reg.get(cfgBackend)) {
          reg.setActiveName(cfgBackend);
        }
      ...
      console.warn("provider select init failed", e);
    }
    ...
    (function initEditorSettingsPanel() {
      ...
      elTop.value = String(Math.round((autoCfg.safeTopRatio ?? 0.2) * 100));
      elBottom.value = String(Math.round((autoCfg.safeBottomRatio ?? 0.6) * 100));
      ...
      function applyFromUI() {
        ...
        if (topPct >= bottomPct - 5) {
          bottomPct = Math.min(90, topPct + 5);
        }
        elTop.value = String(Math.round(topPct));
        elBottom.value = String(Math.round(bottomPct));
        setLabels();
        const newAuto = {
          enabled: !!elAuto.checked,
          safeTopRatio: topPct / 100,
          safeBottomRatio: bottomPct / 100,
          behavior: window.APP_CONFIG.editor.autoScroll.behavior || "smooth",
        };
        const newFx = {
          enabled: !!elInk.checked,
          durationMs: Math.max(0, elInkDur.valueAsNumber || 160),
        };
        ...
        // persist
        try {
          window.StorageUtil?.saveJSON?.("agp_editor_ui_cfg", {
            autoScroll: newAuto,
            inputFx: newFx,
          });
        } catch {}
    ...
    function ensureCaretVisible() {
      ...
      let target = null;
      if (caretY < topBound) {
        target = Math.max(0, caretY - viewH * 0.35);
      } else if (caretY > bottomBound) {
        target = Math.max(0, caretY - viewH * 0.65);
      }
      if (target != null) {
        window.scrollTo({ top: target, behavior: autoCfg.behavior || "auto" });
      }
    }
    ...
    function applyInk(kind) {
      ...
      if (window.anime && inkOverlay) {
        ...
        let x = ed.width * 0.5,
          y = ed.height * 0.5;
        if (crect) {
          x = crect.left + crect.width / 2 - ed.left;
          y = crect.top + crect.height / 2 - ed.top;
        }
        ...
        try {
          window.anime.remove(inkOverlay);
        } catch {}
    ...
    editor.addEventListener("input", (e) => {
      debouncedEnsure();
      const t = e.inputType || "";
      if (t.startsWith("delete")) applyInk("out");
      else applyInk("in");
    });
    ...
    document.getElementById("btn-save-full").addEventListener("click", async () => {
      ...
      if (bridgeEnabled() && window.StorageBridge) {
        await window.StorageBridge.saveFull(obj);
      } else {
        StorageUtil.saveJSON(KEY_FULL, obj);
      }
      alert("完全保存しました");
    });
    document.getElementById("btn-load-full").addEventListener("click", async () => {
      const obj =
        bridgeEnabled() && window.StorageBridge
    ...
    document.getElementById("btn-export").addEventListener("click", () => {
      if (window.ExportImport?.createExportDialog) {
        document.body.appendChild(window.ExportImport.createExportDialog());
      } else {
        alert("拡張エクスポート機能が利用できません");
      }
    });
    (function initEnhancedImport() {
      const input = document.getElementById("file-import");
      if (input) {
        input.addEventListener("change", () => {
          if (window.ExportImport?.createImportDialog) {
            document.body.appendChild(window.ExportImport.createImportDialog());
          } else {
            alert("拡張インポート機能が利用できません");
          }
        });
        // ラベルクリック時のフォールバック
        try {
    ...
    function __ensureGameImportUI() {
      try {
        ...
        if (!input) {
          ...
          input.id = "file-import-game";
          input.style.display = "none";
          (container || document.body).appendChild(input);
          try {
            console.debug("[admin] added hidden #file-import-game");
          } catch {}
        }
        if (!container) {
        ...
        if (!btn) {
          ...
          try {
            console.debug("[admin] added import button in NodeEditor actions");
          } catch {}
        }
        ...
        if (!container.querySelector('label.file-label[data-kind="game-import"]')) {
          ...
          try {
            console.debug("[admin] added label fallback for game import");
          } catch {}
        }
        ...
        if (header && !document.getElementById("ne-import-game-btn-header")) {
          ...
          try {
            console.debug("[admin] added header fallback button for game import");
          } catch {}
        }
      } catch (e) {
        console.warn("ensureGameImportUI failed", e);
      }
    }
    ...
    try {
      ...
      if (tgl) {
        tgl.addEventListener("click", () => setTimeout(__ensureGameImportUI, 0));
      }
    } catch {}
          ...
          try {
            ...
            if (window.Validator?.validateGameSpec) {
              ...
              if (v.warnings && v.warnings.length) {
                console.warn("GameSpec warnings:", v.warnings);
              }
            }
            ...
            const engineData = window.Converters?.normalizeSpecToEngine
              ...
              : (function () {
                  const title =
                    spec?.meta?.title || spec?.title || (APP_CONFIG?.strings?.defaultTitle ?? "");
            ...
            try {
              document.getElementById("ne-load")?.click();
            } catch {}
            alert("ゲームJSONをインポートしました（プレイ画面/NodeEditorで使用されます）");
          } catch (e) {
            console.error(e);
            alert("ゲームJSON の読み込み/変換に失敗しました");
          }
        };
    ...
    async function loadById(id) {
      const { simple, full } = window.APP_CONFIG?.storage?.keys || {
        simple: KEY_SIMPLE,
        full: KEY_FULL,
      };
      try {
        if (bridgeEnabled() && window.StorageBridge) {
          if (id === full) {
            const obj = await window.StorageBridge.loadFull();
            if (!obj) return false;
            applyFull(obj);
            return true;
          }
          ...
          if (id === full) {
            const obj = StorageUtil.loadJSON(KEY_FULL);
            if (!obj) return false;
            applyFull(obj);
            return true;
          }
          if (id === simple) {
            const text = StorageUtil.loadText(KEY_SIMPLE, "");
            applySimple(text);
            return true;
          }
    ...
    async function deleteById(id) {
      try {
        if (bridgeEnabled() && window.StorageBridge) {
          await window.StorageBridge.remove(id);
          return true;
        }
        if (window.StorageHub?.getProvider) {
          const r = window.StorageHub.getProvider().remove(id);
    ...
    (function initLazyLoading() {
      // Load export/import on first interaction
      document.getElementById("btn-export")?.addEventListener("click", async () => {
        await window.LazyLoader.loadScript("scripts/exportImport.js");
      });
      document.querySelector('label[for="file-import"]')?.addEventListener("click", async () => {
        await window.LazyLoader.loadScript("scripts/exportImport.js");
      });

      // Load NodeEditor scripts when accordion opens
      document.querySelector("#acc-node-editor summary")?.addEventListener("click", async () => {
        await window.LazyLoader.loadScripts([
      ...
      // Load Mermaid scripts when accordion opens
      document.querySelector("#acc-mermaid summary")?.addEventListener("click", async () => {
        await window.LazyLoader.loadScript("scripts/mermaidPreview.js");
      });

      // Load SavePreview when needed
      const lazyLoader = window.LazyLoader.createLazyLoader(async () => {
        await window.LazyLoader.loadScript("scripts/savePreview.js");
      });
      document.querySelectorAll("#preview-panel, #saves-panel").forEach((el) => {
