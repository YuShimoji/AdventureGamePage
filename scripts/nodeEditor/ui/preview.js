(function(){
  ...
  function updatePreview(node, specData){
    const { previewTitle, previewText, previewImage, previewChoices, gamePreview, mermaidPreview } = readUIRefs();
    if(!node){
    ...
    if(previewImage) {
      if(node.image){
        previewImage.innerHTML = `<img src="${node.image}" alt="Node image" />`;
  ...
  function updateMermaidPreview(specData){
    ...
    if(window.mermaid && window.MermaidPreviewLogicManager?.ensureMermaid()){
      window.mermaid.render(`ne-modal-${Date.now()}`, code).then(result => {
  ...
  function updateParallelPreview(node, specData){
    const { parallelPreviewTitle, parallelPreviewText, parallelPreviewImage, parallelPreviewChoices, parallelGamePreview, parallelMermaidPreview } = readUIRefs();
    if(!node){
    ...
    if(parallelPreviewImage) {
      if(node.image){
        parallelPreviewImage.innerHTML = `<img src="${node.image}" alt="Node image" />`;
        parallelPreviewImage.hidden = false;
      } else {
        parallelPreviewImage.hidden = true;
      }
    }
    ...
    // Update Mermaid preview if in mermaid mode
    if(parallelPreviewMode === 'mermaid' && parallelMermaidPreview && !parallelMermaidPreview.hidden){
  ...
  function updateParallelMermaidPreview(specData){
    ...
    if(window.mermaid && window.MermaidPreviewLogicManager?.ensureMermaid()){
      window.mermaid.render(`ne-parallel-${Date.now()}`, code).then(result => {
