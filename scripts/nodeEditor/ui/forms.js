(function () {
  ...
  function renderActions(node, specData, callbacks) {
    ...
    arr.forEach((action, idx) => {
      ...
      row.className = 'ne-action-row';

      let lastWarnedAudioUrl = null;

      const typeSel = document.createElement('select');
      typeSel.className = 'ne-action-type';
      const types = [
        'add_item',
        'remove_item',
        'use_item',
      ...
      itemIdContainer.className = 'ne-action-item-container';
      const itemIdLabel = document.createElement('span');
      itemIdLabel.textContent = 'アイテムID';
      const itemId = document.createElement('input');
      itemId.type = 'text';
      itemId.placeholder = 'アイテムID';
      itemId.value = action.itemId || '';
      const itemIdDropZone = document.createElement('div');
      itemIdDropZone.className = 'ne-action-item-drop-zone';
      itemIdDropZone.textContent = 'アイテムをドロップ';
      ...
      quantity.min = '1';
      quantity.placeholder = '個数';
      quantity.value = action.quantity || 1;
      ...
      audioUrl.value = action.url || '';
      const audioVolume = document.createElement('input');
      audioVolume.type = 'range';
      audioVolume.min = '0';
      audioVolume.max = '1';
      audioVolume.step = '0.1';
      audioVolume.value = action.volume || 1;
      const audioVolumeLabel = document.createElement('span');
      audioVolumeLabel.textContent = `音量: ${(action.volume || 1) * 100}%`;
      const audioLoop = document.createElement('label');
      audioLoop.className = 'ne-action-loop';
      const audioLoopInput = document.createElement('input');
      audioLoopInput.type = 'checkbox';
      audioLoopInput.checked = action.loop !== false;
      audioLoop.appendChild(audioLoopInput);
      audioLoop.appendChild(document.createTextNode(' ループ再生'));
      const audioFadeIn = document.createElement('label');
      audioFadeIn.className = 'ne-action-fadein';
      const audioFadeInInput = document.createElement('input');
      audioFadeInInput.type = 'checkbox';
      audioFadeInInput.checked = action.fadeIn !== false;
      audioFadeIn.appendChild(audioFadeInInput);
      audioFadeIn.appendChild(document.createTextNode(' フェードイン'));
      const audioCrossfade = document.createElement('label');
      audioCrossfade.className = 'ne-action-crossfade';
      const audioCrossfadeInput = document.createElement('input');
      audioCrossfadeInput.type = 'checkbox';
      audioCrossfadeInput.checked = action.crossfade !== false;
      audioCrossfade.appendChild(audioCrossfadeInput);
      audioCrossfade.appendChild(document.createTextNode(' クロスフェード'));
      const audioFadeOut = document.createElement('label');
      audioFadeOut.className = 'ne-action-fadeout';
      const audioFadeOutInput = document.createElement('input');
      audioFadeOutInput.type = 'checkbox';
      audioFadeOutInput.checked = action.fadeOut !== false;
      audioFadeOut.appendChild(audioFadeOutInput);
      audioFadeOut.appendChild(document.createTextNode(' フェードアウト'));

      // use_item専用の設定
      const consumeCheck = document.createElement('label');
      consumeCheck.className = 'ne-action-consume';
      const consumeInput = document.createElement('input');
      consumeInput.type = 'checkbox';
      consumeInput.checked = action.consume !== false;
      consumeCheck.appendChild(consumeInput);
      consumeCheck.appendChild(document.createTextNode(' 使用時に消費'));

      const effectTypeSel = document.createElement('select');
      effectTypeSel.className = 'ne-action-effect-type';
      const effectTypes = ['show_text', 'set_variable', 'custom'];
      effectTypes.forEach(type => {
        const opt = document.createElement('option');
        opt.value = type;
        opt.textContent = type;
        effectTypeSel.appendChild(opt);
      });
      if (action.effect) {
      ...
      effectInput.placeholder = '効果内容';
      if (action.effect) {
        effectInput.value = action.effect.text || action.effect.key || '';
      }

      // set_variable専用の設定
      const variableKey = document.createElement('input');
      variableKey.type = 'text';
      variableKey.placeholder = '変数キー';
      variableKey.value = action.key || '';
      const variableOperation = document.createElement('select');
      variableOperation.className = 'ne-action-operation';
      ['set', 'add', 'subtract', 'multiply', 'divide'].forEach(op => {
        const opt = document.createElement('option');
        opt.value = op;
        opt.textContent = op;
        variableOperation.appendChild(opt);
      });
      variableOperation.value = action.operation || 'set';
      const variableValue = document.createElement('input');
      variableValue.type = 'text';
      variableValue.placeholder = '値';
      variableValue.value = action.value !== undefined ? action.value : '';
      ...
      function updateAction() {
        ...
        if (action.type.startsWith('play_') || action.type.startsWith('stop_')) {
          ...
          const urlRef = (action.url || '').trim();
          if (urlRef && urlRef !== lastWarnedAudioUrl && window.MediaResolver?.resolveAudioUrl) {
            const resolved = window.MediaResolver.resolveAudioUrl(urlRef);
            if (resolved && resolved.ok === false) {
              lastWarnedAudioUrl = urlRef;
              const msg = '音声URLがポリシーによりブロックされました（相対パス / DataURL を推奨）';
              if (window.ToastManager) ToastManager.warning(msg);
            }
          }
        }
      ...
      typeSel.addEventListener('change', () => {
        ...
        // オーディオ関連のフィールドを表示
        const isAudioAction =
          typeSel.value.startsWith('play_') || typeSel.value.startsWith('stop_');
        ...
        audioFadeIn.style.display =
          isAudioAction && typeSel.value === 'play_bgm' ? 'block' : 'none';
        audioCrossfade.style.display =
          isAudioAction && typeSel.value === 'play_bgm' ? 'block' : 'none';
        audioFadeOut.style.display =
          isAudioAction && typeSel.value === 'stop_bgm' ? 'block' : 'none';
  ...
  function renderChoices(node, specData, callbacks) {
    ...
    arr.forEach((choice, idx) => {
      ...
      const row = document.createElement('div');
      row.className = 'ne-choice-row';

      const label = document.createElement('input');
      label.type = 'text';
      label.placeholder = '選択肢のテキスト';
      label.value = c.label || c.text || '';
      const target = document.createElement('input');
      target.type = 'text';
      target.placeholder = 'ターゲットノードID';
      target.value = c.target || c.to || '';
      target.setAttribute('list', 'ne-node-id-list');

      // Conditions section
      const conditionsContainer = document.createElement('div');
      conditionsContainer.className = 'ne-conditions';
      const conditionsHeader = document.createElement('div');
      conditionsHeader.className = 'ne-conditions-header';
      const conditionsToggle = document.createElement('button');
      conditionsToggle.className = 'btn btn-sm';
      conditionsToggle.textContent = '条件';
      const conditionsList = document.createElement('div');
      conditionsList.className = 'ne-conditions-list';
      conditionsList.hidden = true;
      ...
      function renderConditions() {
        ...
        condArr.forEach((cond, condIdx) => {
          ...
          condRow.className = 'ne-condition-row';

          const condTypeSel = document.createElement('select');
          condTypeSel.className = 'ne-condition-type';
          const condTypes = [
            'has_item',
            'item_count',
            'inventory_empty',
          ...
          const condItemId = document.createElement('input');
          condItemId.type = 'text';
          condItemId.placeholder = 'itemId/変数キー';
          condItemId.value = cond.itemId || cond.key || '';
          const condOperator = document.createElement('select');
          condOperator.className = 'ne-condition-operator';
          ['>=', '>', '==', '!=', '<=', '<', '===', '!=='].forEach(op => {
            const opt = document.createElement('option');
            opt.value = op;
            opt.textContent = op;
            condOperator.appendChild(opt);
          });
          condOperator.value = cond.operator || '>=';
          const condCount = document.createElement('input');
          condCount.type = 'text';
          condCount.placeholder = '値';
          condCount.value =
            cond.count !== undefined ? cond.count : cond.value !== undefined ? cond.value : 0;
          const condDel = document.createElement('button');
          condDel.className = 'btn btn-sm';
          condDel.textContent = '×';
          ...
          function updateCondition() {
            ...
            if (cond.type.startsWith('variable_')) {
              ...
              cond.value =
                condCount.value === ''
                  ? undefined
                  : isNaN(condCount.value)
                    ? condCount.value
                    : parseFloat(condCount.value);
              delete cond.itemId;
              delete cond.count;
            } else {
      ...
      del.addEventListener('click', () => {
        window.NodeEditorUtils.removeChoice(node, idx);
        renderChoices(node, specData, callbacks);
        setDirty(true);
        refreshUnresolvedPanel();
        notifySpecUpdated();
      });
      up.addEventListener('click', () => {
        window.NodeEditorUtils.moveChoice(node, idx, Math.max(0, idx - 1));
        renderChoices(node, specData, callbacks);
        setDirty(true);
        notifySpecUpdated();
      });
      down.addEventListener('click', () => {
        window.NodeEditorUtils.moveChoice(node, idx, Math.min(arr.length - 1, idx + 1));
        renderChoices(node, specData, callbacks);
        setDirty(true);
        notifySpecUpdated();
      });
