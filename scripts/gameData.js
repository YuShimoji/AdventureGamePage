(function(){
  ...
  function engineToSpec(engine){
    if(window.Converters?.engineToSpec){ return window.Converters.engineToSpec(engine); }
    ...
    return { version:'1.0', meta:{ title: engine.title||'Adventure', author:'', createdAt: nowISO(), updatedAt: nowISO(), start: engine.start||'start' }, nodes };
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if(btn) btn.addEventListener('click', () => {
      ...
      const title = (titleInput?.value || '').trim() || (window.APP_CONFIG?.strings?.defaultTitle ?? 'Untitled');
      const data = {
        version: '1.0',
        meta: {
          title,
    ...
    if(btnCurrent) btnCurrent.addEventListener('click', () => {
      ...
      if(!spec){ alert('エクスポート可能なゲームデータがありません'); return; }
