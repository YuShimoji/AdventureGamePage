    ...
    getUserMessage(errorType, context = {}) {
      const messages = {
        [this.ErrorType.STORAGE_FULL]: {
          title: 'ストレージ容量不足',
          message: 'ブラウザのストレージ容量が不足しています。古いデータを削除するか、データをエクスポートして空き容量を確保してください。',
          actions: ['古いデータを削除', 'データをエクスポート', 'キャンセル']
        },
        [this.ErrorType.STORAGE_UNAVAILABLE]: {
          title: 'ストレージ利用不可',
          message: 'ブラウザのストレージ機能が利用できません。プライベートブラウジングモードを無効にするか、ブラウザの設定を確認してください。',
          actions: ['再試行', 'キャンセル']
        },
    ...
    async withRetry(operation, maxRetries = 3, delayMs = 1000) {
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
          ...
          if (errorType === this.ErrorType.VALIDATION_ERROR || 
