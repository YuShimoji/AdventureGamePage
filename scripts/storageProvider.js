(function(){
  ...
  class StorageProvider {
    ...
    // Saves data by id
    set(id, data){ throw new Error('Not implemented'); }
    // Removes data by id
  ...
  class LocalStorageProvider extends StorageProvider {
    constructor(storage){ super(); this.storage = storage || window.localStorage; }
    ...
    list(){
      ...
      for(const k of [kSimple, kFull]){
        ...
        if(k === kFull){
          ...
          } catch(e){ console.warn('LocalStorageProvider.list parse error:', e); }
        } else if(k === kSimple){
          const text = raw;
          items.push({
            id: k,
          ...
          for (let i = 0; i < this.storage.length; i++) {
            ...
            try {
              ...
              items.push({
                id: key,
                title: obj.title || '(無題)'
                  + (obj.meta?.label ? ` - ${obj.meta.label}` : ''),
                savedAt: obj.meta?.savedAt || null,
                size: byteSize(raw),
                excerpt: toExcerpt(text),
                type: 'snapshot'
              });
            } catch(e){ /* ignore broken snapshot */ }
          }
        } catch(e){ console.warn('LocalStorageProvider.snapshot list error', e); }
      }
      return items;
    }
    ...
    remove(id){ this.storage.removeItem(id); }
  }
  ...
  class IndexedDBProvider extends StorageProvider {
    constructor(dbName = 'agp_db', storeName = 'manuscripts'){
      super();
      this.dbName = dbName; this.storeName = storeName; this._opening = null; this._db = null;
      ...
      try {
        ...
        for(const k of [kSimple, kFull]){
          ...
          if(k === kFull){
            ...
            items.push({ id: k, title: v.title || '(無題)', savedAt: v.meta?.savedAt || null, size: byteSize(v), excerpt: toExcerpt(text), type: 'full' });
          } else if(k === kSimple){
            const text = typeof v === 'string' ? v : (v.text ?? '');
            items.push({ id: k, title: '(シンプル保存)', savedAt: null, size: byteSize(text), excerpt: toExcerpt(text), type: 'simple' });
          }
        }
        // snapshots
        const snapConf = window.APP_CONFIG?.storage?.snapshots;
        ...
        if (snapConf?.enabled) {
          ...
          for(const key of keys){
            ...
            if(!v) continue;
            const text = v.text ?? stripHtml(v.html ?? '');
            items.push({ id: key, title: v.title || '(無題)'
              + (v.meta?.label ? ` - ${v.meta.label}` : ''), savedAt: v.meta?.savedAt || null, size: byteSize(v), excerpt: toExcerpt(text), type: 'snapshot' });
          }
        }
      } catch(e){ console.error('IndexedDBProvider.list', e); }
      return items;
    }
    ...
    async get(id){
      try {
        ...
        const val = await new Promise((resolve, reject) => { const r = store.get(id); r.onsuccess = () => resolve(r.result?.value ?? null); r.onerror = () => reject(r.error); });
        return val;
      } catch(e){ console.error('IndexedDBProvider.get', e); return null; }
    }

    async set(id, data){
      try {
        const store = await this._tx('readwrite');
        await new Promise((resolve, reject) => { const r = store.put({ id, value: data }); r.onsuccess = () => resolve(true); r.onerror = () => reject(r.error); });
      } catch(e){ console.error('IndexedDBProvider.set', e); }
    }

    async remove(id){
      try {
        const store = await this._tx('readwrite');
        await new Promise((resolve, reject) => { const r = store.delete(id); r.onsuccess = () => resolve(true); r.onerror = () => reject(r.error); });
      } catch(e){ console.error('IndexedDBProvider.remove', e); }
    }
    ...
    async _allKeys(){
      try {
        ...
        if('getAllKeys' in store){
          return await new Promise((resolve, reject)=>{ const r = store.getAllKeys(); r.onsuccess = ()=> resolve(r.result || []); r.onerror=()=>reject(r.error); });
        }
        const keys = [];
        await new Promise((resolve, reject) => {
          const req = store.openCursor();
          req.onsuccess = () => { const cursor = req.result; if(cursor){ keys.push(cursor.key); cursor.continue(); } else resolve(true); };
          req.onerror = () => reject(req.error);
        });
        return keys;
      } catch(e){ console.error('IndexedDBProvider._allKeys', e); return []; }
    }
  }
  ...
  function byteSize(v){
    try { return new Blob([typeof v === 'string' ? v : JSON.stringify(v)]).size; } catch { return (v?.length ?? 0); }
  }
