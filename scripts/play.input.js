(function () {
  ...
  window.PlayInput = {
    ...
    setupMobileMenu: function() {
      ...
      if (mobileBtnSave) mobileBtnSave.addEventListener('click', () => {
        const btnSave = document.getElementById('btn-save');
        if (btnSave) btnSave.click();
        closeMobileMenu();
      });
      if (mobileBtnLoad) mobileBtnLoad.addEventListener('click', () => {
        const btnLoad = document.getElementById('btn-load');
        if (btnLoad) btnLoad.click();
        closeMobileMenu();
      });
      if (mobileBtnInventory) mobileBtnInventory.addEventListener('click', () => {
        const btnInventory = document.getElementById('btn-inventory');
        if (btnInventory) btnInventory.click();
        closeMobileMenu();
      });
      if (mobileBtnBack) mobileBtnBack.addEventListener('click', () => {
        if (this.engine.goBack()) {
          // announce action if needed
        }
        closeMobileMenu();
      });
      if (mobileBtnForward) mobileBtnForward.addEventListener('click', () => {
        if (this.engine.goForward()) {
          // announce action if needed
        }
        closeMobileMenu();
      });
      if (mobileBtnRestart) mobileBtnRestart.addEventListener('click', () => {
      ...
      if (mobileBtnTheme) mobileBtnTheme.addEventListener('click', () => {
    ...
    setupAccessibility: function() {
      ...
      const navigateChoices = (direction) => {
        ...
        if (direction === 'next') {
          ...
          currentFocusIndex = currentFocusIndex <= 0 ? choiceButtons.length - 1 : currentFocusIndex - 1;
        }
