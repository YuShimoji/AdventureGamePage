(function () {
  ...
  function renderChoices(node) {
    ...
    arr.forEach((c, idx) => {
      ...
      up.addEventListener("click", () => {
        moveChoice(node, idx, Math.max(0, idx - 1));
        renderChoices(node);
        setDirty(true);
        notifySpecUpdated();
      });
      down.addEventListener("click", () => {
        moveChoice(node, idx, Math.min(arr.length - 1, idx + 1));
        renderChoices(node);
        setDirty(true);
        notifySpecUpdated();
      });
  ...
  function refreshStartSelect() {
    ...
    if (specData?.meta?.start && findNodeById(specData.nodes, specData.meta.start))
      startSel.value = specData.meta.start;
  }

  function refreshNodeIdDatalist() {
    const dl = document.getElementById("ne-node-id-list");
    if (!dl) return;
    dl.innerHTML = "";
    (specData.nodes || []).forEach((n) => {
      const opt = document.createElement("option");
  ...
  async function loadFromStorage() {
    try {
      ...
      if (!raw) {
        alert("agp_game_data がありません。ゲームJSONをインポートしてから取り込んでください。");
        return;
      }
      const spec = window.Converters?.normalizeEngineToSpec
        ? window.Converters.normalizeEngineToSpec(raw)
        : null;
      if (!spec) throw new Error("normalizeEngineToSpec 変換に失敗");
      specData = normalizeSpec(spec);
      originalSpec = JSON.parse(JSON.stringify(specData));
      refreshNodeList();
      refreshExportList();
    ...
    if (window.Validator?.validateGameSpec) {
      ...
      try {
        ...
        if (panel && listEl) {
          ...
          warnDetails
            .filter((w) => w && w.code !== "WARN_UNRESOLVED_TARGET")
            .forEach((w) => {
              const li = document.createElement("li");
              li.className = "warn";
  ...
  function saveToStorage() {
    const diffPanel = document.getElementById("ne-diff-panel");
    const diffContent = document.getElementById("ne-diff-content");
    const diffApply = document.getElementById("ne-diff-apply");
    const diffCancel = document.getElementById("ne-diff-cancel");

    if (!originalSpec) {
      originalSpec = JSON.parse(JSON.stringify(specData));
    }

    const diffs = computeDiff(originalSpec, specData);
    if (diffs.length === 0) {
      alert("変更がありません");
      return;
    }
    ...
    // Show diff panel
    if (diffContent) diffContent.textContent = diffs.join("\n");
    ...
    const applyHandler = () => {
      try {
        const engine = window.Converters?.normalizeSpecToEngine
          ? window.Converters.normalizeSpecToEngine(specData)
          : null;
    ...
    if (diffApply) {
      diffApply.removeEventListener("click", applyHandler);
      diffApply.addEventListener("click", applyHandler);
    }
    if (diffCancel) {
      diffCancel.removeEventListener("click", cancelHandler);
      diffCancel.addEventListener("click", cancelHandler);
    }
  }
  ...
  function bindUI() {
    ...
    if (id)
      id.addEventListener("change", () => {
        ...
        const node = findNodeById(specData.nodes, curId);
        if (!node) return;
        const ids = (specData.nodes || []).filter((n) => n !== node).map((n) => n.id);
        let nv = id.value.trim();
        if (!nv) nv = ensureUniqueId("node:new", ids);
        // 重複IDの場合は自動でユニーク化
        if (ids.includes(nv)) nv = ensureUniqueId(nv, ids);
        if (nv !== node.id) {
          // start の更新（もし同一なら）
          if (specData?.meta?.start === node.id) {
            specData.meta.start = nv;
          }
          // 参照更新（target）
          const oldId = node.id;
          const newId = nv;
          let refCount = 0;
          try {
            refCount = renameReferences(specData, oldId, newId);
          } catch {}
          if (refCount > 0) {
            const ok = confirm(
              `IDを変更しました。${refCount} 件の参照(target)を更新します。よろしいですか？`
            );
            if (!ok) {
              // revert references if user cancels
    ...
    if (text)
      text.addEventListener("input", () => {
        const { sel } = readUIRefs();
        const node = findNodeById(specData.nodes, sel.value);
        if (!node) return;
    ...
    if (exportSelectAll)
      exportSelectAll.addEventListener("click", () => {
        if (!exportMulti) return;
        for (const opt of exportMulti.options) {
          opt.selected = true;
        }
      });
    if (exportSelectNone)
    ...
    if (exportPartial)
      exportPartial.addEventListener("click", () => {
        if (!exportMulti) return;
        ...
        try {
          ...
          const a = document.createElement("a");
          a.href = URL.createObjectURL(blob);
          a.download = "game-spec.partial.json";
          a.click();
          URL.revokeObjectURL(a.href);
        } catch (e) {
    ...
    document.addEventListener("keydown", (ev) => {
      ...
      if (ev.ctrlKey && ev.key === "s") {
        ...
        ev.preventDefault();
        btnAddChoice?.click();
      } else if (!ev.ctrlKey && !ev.altKey && ev.key === "Delete") {
        ev.preventDefault();
        btnRemoveNode?.click();
      }
    });
  ...
  function computeUnresolvedTargets(spec) {
    ...
    nodes.forEach((n) => {
      ...
      arr.forEach((c, idx) => {
        ...
        if (!findNodeById(nodes, c.target))
          out.push({ fromId: n.id, index: idx, target: c.target });
      });
    });
    return out;
  }
  function refreshUnresolvedPanel() {
    const { unresolvedPanel, unresolvedList } = readUIRefs();
    if (!unresolvedPanel || !unresolvedList) return;
  ...
  function collectSubgraph(spec, seedIds) {
    ...
    while (queue.length) {
      ...
      (n.choices || []).forEach((c) => {
        if (c && c.target && idMap.has(c.target) && !seen.has(c.target)) {
          seen.add(c.target);
          queue.push(c.target);
        }
      });
    ...
    return {
      ...
      meta: {
        ...(base.meta || {}),
        start: (seedIds && seedIds[0]) || base.meta?.start || filtered[0]?.id || "start",
      },
      nodes: filtered,
    };
  }
  ...
  function computeDiff(oldSpec, newSpec) {
    const diffs = [];
    const oldNodes = oldSpec?.nodes || {};
    const newNodes = newSpec?.nodes || {};
    const allIds = new Set([...Object.keys(oldNodes), ...Object.keys(newNodes)]);

    for (const id of allIds) {
      const oldNode = oldNodes[id];
      const newNode = newNodes[id];
      if (!oldNode && newNode) {
        diffs.push(`+ ノード "${id}" を追加`);
      } else if (oldNode && !newNode) {
        diffs.push(`- ノード "${id}" を削除`);
      } else if (oldNode && newNode) {
        const oldText = oldNode.text || "";
        const newText = newNode.text || "";
        if (oldText !== newText) {
          diffs.push(`~ ノード "${id}" の本文変更`);
        }
        const oldChoices = (oldNode.choices || [])
          .map((c) => `${c.text || ""} -> ${c.to || ""}`)
          .sort();
        const newChoices = (newNode.choices || [])
          .map((c) => `${c.text || ""} -> ${c.to || ""}`)
          .sort();
        const oldChoicesStr = oldChoices.join("\n");
        const newChoicesStr = newChoices.join("\n");
        if (oldChoicesStr !== newChoicesStr) {
          diffs.push(`~ ノード "${id}" の選択肢変更`);
        }
      }
    }
    return diffs;
