(function () {
  ...
  function getProvider() {
    return window.StorageProviders?.Registry?.getActive?.();
  }

  async function saveSimple(text) {
    const p = getProvider();
    if (!p || !p.isAvailable()) throw new Error("No provider");
    const rec = {
      id: undefined,
      type: "manuscript",
      kind: "simple",
      title: window.APP_CONFIG?.strings?.defaultTitle || "",
      text: text || "",
      html: undefined,
      meta: { theme: window.Theme?.loadTheme?.() },
    };
    const id = await p.save(rec);
    try {
      localStorage.setItem(PTR_SIMPLE, id);
    } catch {}
    return id;
  }
  async function loadSimple() {
    ...
    let id = null;
    try {
      id = localStorage.getItem(PTR_SIMPLE);
    } catch {}
    if (!id) {
      // fallback to latest simple
      const list = await p.list();
      const item =
        (list || []).find((x) => x.kind === "simple") ||
        (list || []).find((x) => x.textPreview != null);
      if (!item) return "";
      id = item.id;
    }
  ...
  async function saveFull(obj) {
    try {
      ...
      const id = await p.save(rec);
      try {
        localStorage.setItem(PTR_FULL, id);
      } catch {}
      return id;
    } catch (e) {
  ...
  async function loadFull() {
    try {
      ...
      let id = null;
      try {
        id = localStorage.getItem(PTR_FULL);
      } catch {}
      if (!id) {
        const list = await p.list();
