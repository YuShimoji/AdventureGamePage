(function () {
  ...
  async function loadFromStorage() {
    try {
      if (
        window.NodeEditorUIManager.isDirty() &&
        !confirm('未保存の変更があります。破棄して取り込みますか？')
      )
        return;
      ...
      window.NodeEditorUIManager.refreshNodeList(
        normalizedSpec.meta?.start || normalizedSpec.nodes[0]?.id
      );
  ...
  function bindUI() {
    ...
    if (id)
      id.addEventListener('change', () => {
        ...
        if (nv !== node.id) {
          ...
          let refCount = 0;
          try {
            refCount = window.NodeEditorUtils.renameReferences(specData, oldId, newId);
          } catch {}
          if (refCount > 0) {
            const ok = confirm(
    ...
    const { exportMulti, exportSelectAll, exportSelectNone, exportPartial } =
      window.NodeEditorUIManager.readUIRefs();
    if (exportSelectAll)
